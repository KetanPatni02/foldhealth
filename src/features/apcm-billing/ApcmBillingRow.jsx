...
const REASON_SHORT = {
  'No Visit in Last 36 Months': 'No Visit (36 mo)',
  'Diagnosis Not Charted in Last 36 Months': 'Dx Not Charted (36 mo)',
  'Ambiguous ICD-10 from EMR Mapping': 'Ambiguous ICD-10',
};
...
function IcdCell({ icdCodes }) {
  ...
  if (!icdCodes?.length) return <span style={{ color: 'var(--neutral-200)', fontSize: 12 }}>—</span>;
...
export function ApcmBillingRow({ patient, isSelected, onSelect, onTriggerBill, onCommentChange }) {
  const langCode = (patient.language || 'en').toUpperCase();
  const langFull = LANG_MAP[patient.language] || patient.language;

  return (
    <tr className={[styles.row, isSelected ? styles.rowChecked : ''].filter(Boolean).join(' ')}>

      {/* Checkbox */}
      <td className={`${styles.stickyLeft} ${styles.stickyCheck} ${styles.checkTd}`}>
        <Checkbox
          checked={isSelected}
      ...
      <td className={`${styles.stickyLeft} ${styles.stickyMember} ${styles.memberTd}`}>
        <div className={styles.patientCell}>
          <Avatar name={patient.name} variant="patient" size={32} />
      ...
      {/* CPT Code */}
      <td className={styles.td}>
        <span style={{ color: 'var(--neutral-200)', fontSize: 12 }}>—</span>
      </td>
      ...
      {/* Last Encounter */}
      <td className={styles.td}>
        {patient.lastEncounterDate || <span style={{ color: 'var(--neutral-200)' }}>—</span>}
      </td>
      ...
      <td className={styles.td}>
        <div className={styles.providerCell}>
          <Avatar name={patient.renderingProvider} variant="provider" size={24} />
