...
export function AgentRulesDrawer() {
  ...
  const headerRight = (
    <button
      ...
      style={{
        ...
        borderRadius: 20, border: '0.5px solid', cursor: 'pointer', fontSize: 'var(--font-sm)', fontWeight: 500,
  ...
  return (
    <Drawer title="Agent Rules Configuration" onClose={() => setAgentRulesGroupId(null)} headerRight={headerRight}>
      {/* Breadcrumb */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 16, fontSize: 'var(--font-sm)', color: 'var(--neutral-200)' }}>
      ...
      <div style={{ border: '0.5px solid var(--neutral-150)', borderRadius: 8, overflow: 'hidden', marginBottom: 16 }}>
        <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 'var(--font-lg)', fontWeight: 500, color: 'var(--neutral-500)', display: 'flex', alignItems: 'center', gap: 6 }}>
              Care Assistant
              <Badge variant="ai-care" label="AI Agent" />
              <Badge variant="compliance-na" label="FAQ Responder" />
            </div>
            <div style={{ fontSize: 'var(--font-sm)', color: 'var(--neutral-200)', marginTop: 2 }}>
      ...
      <div style={{ border: '0.5px solid var(--neutral-150)', borderRadius: 8, padding: '12px 16px', marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <span style={{ fontSize: 'var(--font-base)', fontWeight: 500, color: 'var(--neutral-500)' }}>Group Settings</span>
          <span style={{ fontSize: 'var(--font-xs)', color: 'var(--neutral-200)', background: 'var(--neutral-50)', padding: '2px 8px', borderRadius: 4 }}>
          ...
          {groupSettings.map((s, i) => (
            <div key={i} style={{ padding: '10px 14px', background: 'var(--neutral-50)', borderRadius: 8, border: '0.5px solid var(--neutral-75, #F3F4F7)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 'var(--font-sm)', color: 'var(--neutral-300)' }}>{s.label}</span>
                {s.isDefault && (
                  <span style={{ fontSize: 'var(--font-2xs)', color: 'var(--neutral-200)', background: 'var(--neutral-0)', padding: '1px 6px', borderRadius: 3, border: '0.5px solid var(--neutral-150)' }}>
                    Default
                  </span>
                )}
              </div>
              <div style={{ fontSize: 'var(--font-base)', fontWeight: 500, color: 'var(--neutral-500)', marginTop: 4 }}>{s.value}</div>
              <div style={{ fontSize: 'var(--font-xs)', color: 'var(--neutral-200)', marginTop: 2 }}>{s.hint}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ border: '0.5px solid var(--neutral-150)', borderRadius: 8, overflow: 'hidden', marginBottom: 16 }}>
        <div style={{ padding: '12px 16px', borderBottom: '0.5px solid var(--neutral-150)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 'var(--font-base)', fontWeight: 500, color: 'var(--neutral-500)' }}>IF / Then Rules</div>
            <span style={{ fontSize: 'var(--font-sm)', color: 'var(--neutral-200)' }}>
        ...
        {showAddRule && (
          <div style={{ padding: '14px 20px', background: 'var(--neutral-50)', borderBottom: '0.5px solid var(--neutral-150)' }}>
            <div style={{ marginBottom: 10 }}>
              <label htmlFor={`${uid}-new-rule-name`} style={{ fontSize: 'var(--font-xs)', fontWeight: 500, color: 'var(--neutral-300)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Rule Name</label>
              <input id={`${uid}-new-rule-name`} value={newRuleName} onChange={e => setNewRuleName(e.target.value)}
                ...
                border: '0.5px solid var(--neutral-150)', fontSize: 'var(--font-md)', fontFamily: "'Inter', sans-serif",
                outline: 'none', boxSizing: 'border-box',
              }} />
            </div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
              <div style={{ flex: 1 }}>
                <label htmlFor={`${uid}-new-rule-condition`} style={{ fontSize: 'var(--font-xs)', fontWeight: 500, color: 'var(--neutral-300)', textTransform: 'uppercase', letterSpacing: '.04em' }}>If (Condition)</label>
              ...
              <div style={{ flex: 1 }}>
                <label htmlFor={`${uid}-new-rule-action`} style={{ fontSize: 'var(--font-xs)', fontWeight: 500, color: 'var(--neutral-300)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Then (Action)</label>
        ...
        {rules.map((r, i) => (
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontSize: 'var(--font-xs)', fontWeight: 500, color: 'var(--neutral-300)', fontVariantNumeric: 'tabular-nums' }}>{i + 1}</span>
            </div>
            ...
            <div style={{ flex: 1, padding: '10px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                ...
                <span style={{ fontWeight: 500, fontSize: 'var(--font-md)', color: 'var(--neutral-500)' }}>{r.name}</span>
                {r.locked && <Icon name="solar:shield-check-linear" size={12} color="var(--neutral-200)" />}
                {r.priority && (
                  <span style={{ fontSize: 'var(--font-2xs)', fontWeight: 500, color: 'var(--status-error)', background: 'var(--status-error-light)', padding: '1px 6px', borderRadius: 3 }}>{r.priority}</span>
                )}
              </div>
              <div style={{ fontSize: 'var(--font-sm)', color: 'var(--neutral-300)', lineHeight: 1.5 }}>
                <span style={{ fontWeight: 500, color: 'var(--primary-300)', fontSize: 'var(--font-xs)', textTransform: 'uppercase', letterSpacing: '.04em' }}>If</span>{' '}
                <span style={{ color: 'var(--neutral-400)' }}>{r.condition}</span>
              </div>
              <div style={{ fontSize: 'var(--font-sm)', color: 'var(--neutral-300)', lineHeight: 1.5, marginTop: 2 }}>
                <span style={{ fontWeight: 500, color: 'var(--status-success)', fontSize: 'var(--font-xs)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Then</span>{' '}
          ...
          {editingRuleId === r.id && !r.locked && (
            <div style={{ padding: '12px 16px', background: 'var(--neutral-50)', borderBottom: '0.5px solid var(--neutral-150)' }}>
              <div style={{ marginBottom: 8 }}>
                <label htmlFor={`${uid}-rule-${r.id}-name`} style={{ fontSize: 'var(--font-xs)', fontWeight: 500, color: 'var(--neutral-300)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Rule Name</label>
                <input id={`${uid}-rule-${r.id}-name`} value={r.name} onChange={e => setRules(prev => prev.map(rule => rule.id === r.id ? { ...rule, name: e.target.value } : rule))}
                  style={{ width: '100%', marginTop: 4, padding: '8px 12px', borderRadius: 4, border: '0.5px solid var(--neutral-150)', fontSize: 'var(--font-md)', fontFamily: "'Inter', sans-serif", outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                <div style={{ flex: 1 }}>
                  <label htmlFor={`${uid}-rule-${r.id}-condition`} style={{ fontSize: 'var(--font-xs)', fontWeight: 500, color: 'var(--neutral-300)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Condition</label>
                ...
                <div style={{ flex: 1 }}>
                  <label htmlFor={`${uid}-rule-${r.id}-action`} style={{ fontSize: 'var(--font-xs)', fontWeight: 500, color: 'var(--neutral-300)', textTransform: 'uppercase', letterSpacing: '.04em' }}>Action</label>
