...
export function ComponentLibraryPanel({ searchQuery = '' }) {
  ...
  // Revisit-proof: only query until the first load lands in the store.
  useEffect(() => {
    if (components.length > 0 || componentsLoading) return;
    fetchEmbedComponents();
  }, [components.length, componentsLoading, fetchEmbedComponents]);
